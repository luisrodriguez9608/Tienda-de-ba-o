...
handleDisconnect(); // Establecer la conexión inicial
...
// Middleware para manejar archivos PHP
...
app.get('/admin', (req, res) => {
  db.query('SELECT productoID, nombre, imagen, precio FROM productos', (err, results) => {
    if (err) {
      console.error('Error al obtener productos: ', err);
      res.status(500).send('Error interno del servidor');
      return;
    }
    const productosConID = results.map(producto => ({ ...producto, productoID: producto.productoID }));
