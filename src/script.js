...
function eliminarProductoDelCarrito(productoId) {
    ...
    .then(response => {
        if (!response.ok) {
            throw new Error('No se encontró el producto en el carrito para eliminar');
        }
        return response.json();
    })
    .then(data => {
    ...
    .catch(error => {
        ...
        alert(error.message || 'Error al eliminar el producto del carrito. Por favor, inténtalo de nuevo.');
    });
}
                ...
                const productoHTML = `
                    <tr>
                        <td class="cart__product__item">
                            <img src="${producto.imagen}" class="cart-product-img" alt="${producto.nombre}">
                        ...
                        <td class="cart__close"><button class="close-btn" data-product-id="${producto.productoID}"><span class="icon_close"></span></button></td>
                    </tr>
                `;
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function obtenerProductosPorCategoria(categoria) {
            fetch(`/productos/por-categoria?categoria=${categoria}`)
                .then(response => response.json())
                .then(productos => {
...
function mostrarProductos(productos) {
    ...
    productos.forEach(producto => {
            ...
            <div class="col-lg-4 col-md-6">
                <div class="product__item">
                    <div class="product__item__pic">
                        ...
                        <div class="label new">New</div>
                        <ul class="product__hover">
                            <li><a href="product-details.html?producto=${producto.id}"><span class="arrow_expand"></span></a></li>
                            <li><a href="#"><span class="icon_heart_alt"></span></a></li>
                            <li><a href="#" class="agregar-al-carrito" data-producto-id="${producto.productoID}"><span class="icon_bag_alt"></span></a></li>
                        </ul>
                    </div>
        ...
        const categorias = document.querySelectorAll('.card-heading a');
...
function agregarProducto() {
    ...
    .then(response => {
        if (!response.ok) {
            throw new Error('Error al agregar el producto. Por favor, inténtalo de nuevo.');
        }
...
// Función para mostrar los productos en el DOM según la categoría seleccionada
function mostrarProductosPorCategoria(categoria) {
    fetch(`/productos-por-categoria?categoria=${categoria}`)
        .then(response => response.json())
        .then(productos => {
            const container = document.getElementById('productos-container-index');
            container.innerHTML = ''; // Limpiar el contenedor antes de agregar los nuevos productos

            productos.forEach(producto => {
                const productoHTML = `
                    <div class="col-lg-3 col-md-4 col-sm-6 mix ${categoria}">
                        <div class="product__item">
                            <div class="product__item__pic">
                                <img src="${producto.imagen}" alt="${producto.nombre}">
                                <div class="label new">New</div>
                                <ul class="product__hover">
                                    <li><a href="${producto.imagen}" class="image-popup"><span class="arrow_expand"></span></a></li>
                                    <li><a href="#"><span class="icon_heart_alt"></span></a></li>
                                    <li><a href="#" class="agregar-al-carrito" data-producto-id="${producto.productoID}"><span class="icon_bag_alt"></span></a></li>
                                </ul>
                            </div>
...
fetch('/productos-por-categoria?categoria=masculino') // Endpoint para obtener productos masculinos
    .then(response => response.json())
    .then(productos => {
        const productosContainer = document.getElementById('productos-container-masculino');
        const productIdSet = new Set(); // Conjunto para almacenar los IDs de productos únicos
    ...
    .catch(error => console.error('Error al obtener productos masculinos: ', error));
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        fetch(`/obtener-producto/${productoID}`)
            ...
            .then(producto => {
                ...
                document.getElementById('product-category').textContent = `Categoría: ${producto.categoria}`;
            })
